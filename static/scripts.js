...
var callBotAgainst = function(){
  updateStatus();
  var botname = document.getElementById("botagainst");
  var selectedbot = botname.options[botname.selectedIndex].value;
  if (selectedbot == 'bot1'){
    getResponseMove();
  } else if (selectedbot == 'bot2'){
    getResponseMoveBot2();
  }
  getPreviousMoves();
}
...
var getResponseMoveBot2 = function() {
  return new Promise((resolve, reject) =>{
    var e = document.getElementById("sel2");
    var depth = e.options[e.selectedIndex].value;
    fen = game.fen()
    previousMoves = getPreviousMoves()
    $.get($SCRIPT_ROOT + "/move/bot2/" + depth + "/"+ fen+"/?pm=" + previousMoves, function(data) {
...
var getbothelp = async function() {
  var botname = document.getElementById("botwith");
  var selectedbot = botname.options[botname.selectedIndex].value;
  updateStatus();
  if (selectedbot == 'bot1'){
    await getResponseMove();
  } else if (selectedbot == 'bot2'){
    await getResponseMoveBot2();
  }
  getPreviousMoves();

  callBotAgainst();
}
var getPreviousMoves = function(){
  var pgn = game.pgn().split(" ");
    var data = [];
    
    for (i = 1; i < pgn.length; i += 3) {
      data.push(pgn[i])
      pgn[i+1] != undefined && data.push(pgn[i+1])
    }
